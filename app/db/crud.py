...
async def check_serviceability(origin: str, dest: str):
    cache_key = f"{SERVICEABILITY_CACHE_VERSION}:serviceability:{origin}:{dest}"
    cached = await cache_get(cache_key)
    if cached:
        return json.loads(cached)
    async with SessionLocal() as session:
        ...
        if origin not in pins or dest not in pins:
            raise ValueError("Invalid origin or destination pincode")
        orig = pins[origin]
        dst = pins[dest]
